...
- `lsp_status.lua` — the lualine statusline LSP-load indicator (✓ ready / ⟳ loading / ✗ expected-but-not-attached / ⏱ kotlin-lsp build EXPIRED / ○ no server, beside the filetype); required at render time from `ui.lua`'s `lualine_x`. Tracks each filetype's single PRIMARY server (see the module's `PRIMARY` table — keep it in sync with `lsp.lua`'s `ensure_installed`, except `kotlin_lsp`, which is intentionally absent from `ensure_installed` because it is self-managed) and waits on LSP work-done progress so ✓ means finished loading, not merely attached. The ⏱ is a kotlin-only special case of ✗: a Kotlin buffer with no `kotlin_lsp` client ~10s after opening triggers a scan of the LSP log tail for `intellij-server has expired`; on a hit it shows ⏱ and pops a one-shot small floating prompt (`M._prompt_expired`, `y`/`n`) offering to run `:KotlinLspUpdate`. It self-heals to ✓/⟳ once a live build attaches.

**Plugin organization by file:**
- `lsp.lua` — nvim-lspconfig + Mason (auto-installs LSP servers) + nvim-cmp (completion). nvim-lspconfig is `lazy = false` on purpose — see the note in the file; lazy-loading it on `BufReadPre` skipped brand-new files, and adding `BufNewFile` breaks filetype detection. `automatic_enable` enables every *installed* server lspconfig knows, so non-servers Mason installs for other reasons (e.g. the `stylua` formatter, which lspconfig also ships an `lsp/` wrapper for) must be listed in its `exclude`. `kotlin_lsp` is deliberately NOT in `ensure_installed` (it is self-managed — see `kotlin.lua`) but stays in the `automatic_enable` exclude.
...
- `kotlin.lua` — kotlin.nvim managing JetBrains kotlin-lsp (excluded from mason-lspconfig auto-enable). **Self-managed, not Mason-installed:** the `intellij-server` is a time-bombed EAP build that expires ~monthly and Mason's registry trails JetBrains by weeks (often only able to reinstall an already-expired build), so `kotlin.lua` points kotlin.nvim's `KOTLIN_LSP_DIR` fallback at `~/.local/share/kotlin-lsp/current` (a symlink to the versioned build). Refresh flow on expiry is one command — **`:KotlinLspUpdate`** (defined here; runs `scripts/update-kotlin-lsp.sh`), which discovers the latest build via the Open VSX `kotlin-server` extension's `server-bundle.json`, downloads + sha256-verifies it, repoints the `current` symlink, and reattaches the server in place, streaming download progress (phase + curl's byte %) to a fidget bar. The script is idempotent (no-op when already current). `lsp_status.lua` surfaces the expiry as ⏱ and pops a small floating prompt that runs the command.
