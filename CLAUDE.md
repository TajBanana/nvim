...
- `kotlin.lua` — kotlin.nvim managing JetBrains kotlin-lsp (excluded from mason-lspconfig auto-enable). **Self-managed, not Mason-installed:** the `intellij-server` is a time-bombed EAP build that expires ~monthly and Mason's registry trails JetBrains by weeks (often only able to reinstall an already-expired build), so `kotlin.lua` points kotlin.nvim's `KOTLIN_LSP_DIR` fallback at `~/.local/share/kotlin-lsp/current` (a symlink to the versioned build). Refresh flow on expiry: find the newest build via the Open VSX `kotlin-server` extension's `server-bundle.json`, drop it in, repoint the symlink. `lsp_status.lua` surfaces the expiry as ⏱.
