...
**Chaining after argument completion.** Accepting a variable, field, property, or constant with `Enter` or `Tab` skips one immediately adjacent closing `)`: `object.method(value|)` becomes `object.method(value)|` (`|` is the cursor). Nested calls skip only the innermost closing parenthesis. Active snippet placeholders, method/function completions, and manual typing do not trigger this. A space or comma before `)` prevents the jump. If you intend to add another argument after accepting a value immediately before `)`, move back inside first.
