...
**Decision.** Take kotlin-lsp out of Mason's `ensure_installed` entirely and self-manage the build: keep it at `~/.local/share/kotlin-lsp/current` (a symlink to a versioned `kotlin-server-<build>/` dir) and let `kotlin.lua` point `KOTLIN_LSP_DIR` there. Refreshing on expiry is then a download plus one `ln -sfn` symlink repoint — no config change.

**How to find a non-expired build when GitHub releases lag.** JetBrains ships newer builds to their **Open VSX / VS Code `kotlin-server` extension** before tagging them on GitHub. Query `https://open-vsx.org/api/JetBrains/kotlin-server` for the latest extension version, download that version's `.vsix` (a zip), and read `extension/server-bundle.json` — it names the server build number, the `.sit` CDN download URL, and a sha256. That is how build `263.4421.0` (extension 0.0.11, 2026-09-02) was located while GitHub still showed only the expired `262.9593.0`.

**Automation.** The refresh is a single command, `:KotlinLspUpdate` (defined in `kotlin.lua`), which runs `scripts/update-kotlin-lsp.sh`: it discovers the latest build from Open VSX, downloads + sha256-verifies it, repoints `current`, prunes old builds, and reattaches the server in place. The script is idempotent (exits early when already current), platform-aware (darwin/linux × arm64/x64), and safe to run with nvim open (it never launches the server, so it can't hit the machine-wide analyzer lock). So upkeep is: see ⏱ → run one command. A scheduled/launchd variant was considered and rejected as unnecessary given the indicator already prompts, and undesirable (silent multi-hundred-MB background downloads).

**Trade-offs.** Still a monthly (manual-trigger) refresh rather than zero-touch, and `:Mason` shows kotlin-lsp oddly (its receipt may still reference an old build) — harmless, since kotlin.nvim resolves the dir itself. There is no supported way to disable the expiry timer (it is compiled into the IntelliJ platform build; clock-rollback and binary-patching are out of scope), so refreshing is the sanctioned free path. If JetBrains ever ships a non-expiring/stable channel, or Mason's registry starts tracking closely, this can revert to plain `ensure_installed`. Detection is automated: `lsp_status.lua` scans the LSP log tail and shows ⏱ + a notification pointing at `:KotlinLspUpdate` (see the indicator decision below).
...
- **⏱ distinguishes an expired kotlin-lsp from a generic failure.** kotlin-lsp dies before attaching when its EAP build expires (see the Kotlin self-managed sub-decision), which otherwise reads as a plain ✗. A `FileType kotlin` autocmd defers ~10s; if no `kotlin_lsp` client has attached by then it reads the tail of the LSP log (`vim.lsp.get_log_path()`, last 64 KB) and, on finding `intellij-server has expired`, shows ⏱ and fires a one-shot `vim.notify` pointing at the refresh procedure. The attached-first check means a healthy build (which attaches within seconds, well before indexing finishes) never reaches the scan, so a stale expiry line from an earlier session cannot false-positive; the flag self-heals to ✓/⟳ once a live build attaches. kotlin is the only server treated this way because it is the only one that expires.
